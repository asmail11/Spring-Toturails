...
@EnableWebSecurity
@Configuration
public class TaxesSecurity extends WebSecurityConfigurerAdapter {
	@Autowired
    ...
    protected void configure(AuthenticationManagerBuilder auth) throws Exception {
    	...
    	.withUser("drmas").password("1234").roles("ADMIN","USER");
    	auth.inMemoryAuthentication()
    	.withUser("issam").password("1234").roles("USER");
    	*/
    	  auth.jdbcAuthentication()
    	  .dataSource(dataSource)
    	  .usersByUsernameQuery("select username as principal, pass as credentials,active from users where username=?")
    	  .authoritiesByUsernameQuery("select username as princpal, role as role from users_roles where username=?")
    	  .rolePrefix("ROLE_")
